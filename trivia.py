import time, tkinter as tk, speech_recognition as sr
...
def _run_offline_mode(quiz):
    _ask_question(quiz)
    answer = quiz.voice_answer()
    time.sleep(5)

    QA.set(quiz.answer)
    window.update()
    agent.say(f"The correct answer is {answer}")
